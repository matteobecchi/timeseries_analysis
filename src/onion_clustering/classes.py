...
class ClusteringObject:
    ...
    def __init__(self, par: Parameters, data: Union[UniData, MultiData]):
        self.par = par
        self.data = data
        self.states: List[Union[StateUni, StateMulti]] = []
        self.iterations = -1
    ...
    def plot_tra_figure(self):
        ...
        for i, t_smooth in enumerate(range(min_t_s, max_t_s + 1, step_t_s)):
            ...
            axesr.set_ylabel('Population of env 0',
                weight='bold', c='#ff7f0e')
...
class ClusteringObject1D(ClusteringObject):
    ...
    def plot_cumulative_figure(self):
        ...
        for state_id, state in enumerate(self.states):
            ...
            axes[1].plot(gaussian(np.linspace(bins[0],
                bins[-1], 1000), *popt),
        ...
        axes[1].hlines(self.states[-1].th_sup[0],
            xmin=0.0, xmax=np.amax(counts),
...
class ClusteringObject2D(ClusteringObject):
    ...
    def plot_input_data(self, filename: str):
        ...
        for dim in range(self.data.dims):
            ...
            counts0, bins0 = np.histogram(flat_m,
                bins=self.par.bins, density=True)
            counts0 *= flat_m.size
        ...
        if self.data.dims == 2:
            ...
            ax3.stairs(counts_selection[1], bin_selection[1], fill=True,
            ...
            for idx, mol in enumerate(m_clean[::step]):
                ax2.plot(mol[:,0], mol[:,1], color='black',
                    lw=0.1, alpha=0.5, rasterized=True)
            ...
            for idx, mol in enumerate(m_clean[::step]):
                ax.plot(mol[:,0], mol[:,1], mol[:,2], color='black',
                    marker='o', ms=0.5, lw=0.2, alpha=1.0, rasterized=True)
    ...
    def preparing_the_data(self):
        ...
        # Print informative messages about trajectory details.
        print('\tTrajectory has ' + str(self.data.num_of_particles) +
            ' particles. ')
        print('\tTrajectory of length ' + str(self.data.num_of_steps) +
            ' frames (' + str(self.data.num_of_steps*t_conv) + ' ' +
            t_units + ').')
        print('\tUsing ' + str(num_windows) + ' windows of length ' +
            str(tau_window) + ' frames (' + str(tau_window*t_conv) + ' '+
            t_units + ').')
    ...
    def plot_cumulative_figure(self):
        ...
        if m_clean.shape[2] == 3:
            ...
            for k in range(3):
                ...
                for i, mol in enumerate(m_resized[::step]):
                    ...
                    ax[a_0][a_1].scatter(mol.T[d_0], mol.T[d_1], c=color_list,
                        cmap=colormap, vmin=0, vmax=n_states-1, s=0.5,
                        rasterized=True)

                    color_list = all_the_labels[id_min]
                    ax[a_0][a_1].plot(m_resized[id_min].T[d_0],
                        m_resized[id_min].T[d_1],
                        c='black', lw=line_w, rasterized=True, zorder=0)
                    ax[a_0][a_1].scatter(m_resized[id_min].T[d_0],
                        m_resized[id_min].T[d_1], c=color_list, cmap=colormap,
                        vmin=0, vmax=n_states-1, s=0.5, rasterized=True)
                    color_list = all_the_labels[id_max]
                    ax[a_0][a_1].plot(m_resized[id_max].T[d_0],
                        m_resized[id_max].T[d_1], c='black', lw=line_w,
                        rasterized=True, zorder=0)
                    ax[a_0][a_1].scatter(m_resized[id_max].T[d_0],
                        m_resized[id_max].T[d_1], c=color_list, cmap=colormap,
                        vmin=0, vmax=n_states-1, s=0.5, rasterized=True)
            ...
            for i, mol in enumerate(m_resized[::step]):
                ax.plot(mol.T[0], mol.T[1], c='black', lw=line_w,
                    rasterized=True, zorder=0)
                color_list = all_the_labels[i*step]
                ax.scatter(mol.T[0], mol.T[1], c=color_list, cmap=colormap,
                    vmin=0, vmax=n_states-1, s=0.5, rasterized=True)
    ...
    def plot_one_trajectory(self):
        ...
        cmap = plt.get_cmap(colormap,
            int(np.max(np.unique(all_the_labels)) -
                np.min(np.unique(all_the_labels)) + 1))
        color = all_the_labels[self.par.example_id]
        ax.plot(signal_x, signal_y, c='black', lw=0.1)

        ax.scatter(signal_x, signal_y, c=color, cmap=cmap,
