...
class Interval(_Constraint):
    ...
    def __contains__(self, val):
        ...
        left_cmp = (
            operator.lt if self.closed in ("left", "both") else operator.le
        )
        right_cmp = (
            operator.gt if self.closed in ("right", "both") else operator.ge
        )
...
class HasMethods(_Constraint):
    ...
    def is_satisfied_by(self, val):
        return all(
            callable(getattr(val, method, None)) for method in self.methods
        )
...
def generate_invalid_param_val(constraint):
    ...
    if isinstance(constraint, Interval) and constraint.type in (
        Real,
        RealNotInt,
    ):
        if constraint.left is not None:
