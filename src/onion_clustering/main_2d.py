...
from onion_clustering.first_classes import MultiData, StateMulti, Parameters
from onion_clustering.functions import read_input_data, custom_fit
from onion_clustering.functions import moving_average_2d
from onion_clustering.functions import relabel_states_2d
...
def gauss_fit_max(
    ...
    """
    print('* Gaussian fit...')
    flat_m = m_clean.reshape((m_clean.shape[0]*m_clean.shape[1],
        m_clean.shape[2]))
    ...
    def find_minima_around_max(data: np.ndarray, max_ind:
        Tuple[int, ...], gap: int):
        """
        ...
        for dim in range(data.ndim):
            ...
            while min_id0 > 0 and \
                data[tuple(tmp_max1)] > data[tuple(tmp_max2)]:
                tmp_max1[dim] -= 1
                tmp_max2[dim] -= 1
                min_id0 -= 1
            ...
            while min_id1 < data.shape[dim] - 1 and \
                data[tuple(tmp_max1)] > data[tuple(tmp_max2)]:
                tmp_max1[dim] += 1
                tmp_max2[dim] += 1
                min_id1 += 1
    ...
    def find_half_height_around_max(data: np.ndarray,
        max_ind: Tuple[int, ...], gap: int):
        """
        ...
        for dim in range(data.ndim):
            ...
            while half_id1 < data.shape[dim] - 1 and \
                data[tuple(tmp_max)] > max_val/2:
                tmp_max[dim] += 1
                half_id1 += 1
    ...
    if m_clean.shape[2] == 2:
        with open(OUTPUT_FILE, 'a', encoding="utf-8") as file:
            print('\n', file=file)
            print(f'\tmu = [{popt[0]:.4f}, {popt[3]:.4f}],'
...
def find_stable_trj(
        m_clean: np.ndarray, tau_window: int, state: StateMulti,
    ...
    with open(OUTPUT_FILE, 'a', encoding="utf-8") as file:
        print(f'\tFraction of windows in state {offset + 1}'
            f' = {fraction_of_points:.3}')
        print(f'\tFraction of windows in state {offset + 1}'
            f' = {fraction_of_points:.3}', file=file)
...
def iterative_search(
    ...
    tmp_labels = np.zeros((cl_ob.data.num_of_particles,
...
def time_resolution_analysis(cl_ob: ClusteringObject2D, full_out: bool):
    ...
    """
    tau_window_list, t_smooth_list = param_grid(cl_ob.par,
        cl_ob.data.num_of_steps)
